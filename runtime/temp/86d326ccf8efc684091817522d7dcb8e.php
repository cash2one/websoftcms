<?php /*a:1:{s:80:"/home/wwwroot/gxwebsoft/public_html/application/mobile/view/module/index/24.html";i:1562404297;}*/ ?>
<div class="old-house house-show-box mt20">
        <div class="title">
            <h3>最新商铺</h3>
            <a href="<?php echo url('ShopsRental/index'); ?>" title="写字楼">更多</a>
        </div>
        <ul>
            <?php $obj = model("shops_rental");$obj = $obj->where("status = 1 and city in ($cityIds) and timeout > $time");$obj = $obj->field("id,title,city,img,acreage,type,renovation,price,tags,update_time");$obj = $obj->order("ordid asc,id desc");$name = $obj->limit(5)->select();if(is_array( $name ) || $name  instanceof \think\Collection || $name instanceof \think\Paginator ): $i = 0; $__LIST__ = $name;if( count($__LIST__)==0 ) : echo "暂无数据" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
            <li>
                <div class="pic">
                ...
                <div class="intro-text">
                    ...
                    <p><span><?php echo getLinkMenuName(18,$vo['type']); ?></span>&nbsp;<span><?php echo getLinkMenuName(8,$vo['renovation']); ?></span>&nbsp;<span><?php echo htmlentities($vo['acreage']); ?><?php echo config('filter.acreage_unit'); ?></span><span>&nbsp;&nbsp;<?php echo getTime($vo['update_time'],'mohu'); ?>更新</span></p>
                    <p class="good">
