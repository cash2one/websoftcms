<?php /*a:4:{s:77:"/home/wwwroot/gxwebsoft/public_html/application/home/view/house/question.html";i:1527591166;s:76:"/home/wwwroot/gxwebsoft/public_html/application/home/view/public/layout.html";i:1541120186;s:72:"/home/wwwroot/gxwebsoft/public_html/application/home/view/house/nav.html";i:1544411302;s:76:"/home/wwwroot/gxwebsoft/public_html/application/home/view/public/footer.html";i:1562384461;}*/ ?>
<!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    <div class="con_wrap comWidth clearfix">
        <div class="leftArea cm_leftArea question">
            <div class="title clearfix">
                <h3><?php echo htmlentities($info['title']); ?>问答</h3>
                ...
                <div class="ipt_box">
                    <form action="#">
                        ...
                        <input type="button" class="btn sub-question" data-uri="<?php echo url('Api/subQuestion'); ?>" value="提交">
                    </form>
                </div>
            </div>
            <!-- 用户问答列表 S -->
            <ul class="question_list list clearfix">
                <?php $obj = model("question");$obj = $obj->where("house_id=$id and status=1");$obj = $obj->field("id,content");$obj = $obj->order("create_time desc");$name = $obj->paginate(5);$pages=$name->render();if(is_array( $name ) || $name  instanceof \think\Collection || $name instanceof \think\Paginator ): $i = 0; $__LIST__ = $name;if( count($__LIST__)==0 ) : echo "暂无数据" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?>
                <li>
                    <div class="ask">
                        <h4><a target="_blank" title="<?php echo htmlentities($data['content']); ?>" href="<?php echo url('Question/detail',['id'=>$data['id']]); ?>"><?php echo htmlentities($data['content']); ?></a></h4>
                    </div>
                    <?php $q_id=$data['id']; $obj = model("answer");$obj = $obj->where("question_id=$q_id and status=1");$obj = $obj->field("count(id) as total,content,create_time");$obj->order("id asc");$name = $obj->find();if( $name ):$answer = $name; ?>
                    <div class="anser">
                        <p><?php echo htmlentities($answer['content']); ?></p>
                    </div>
                    <p class="time"><?php if(!(empty($answer['create_time']) || (($answer['create_time'] instanceof \think\Collection || $answer['create_time'] instanceof \think\Paginator ) && $answer['create_time']->isEmpty()))): ?><?php echo htmlentities(date('Y-m-d H:i',!is_numeric($answer['create_time'])? strtotime($answer['create_time']) : $answer['create_time'])); ?><?php endif; ?></p>
                    <div class="anser_num"><p><?php echo htmlentities($answer['total']); ?>个<br />回答</p></div>
                    <?php else: echo "暂无数据" ;endif; ?>
                </li>
        ...
        <div class="rightArea cm_rightArea ">
            ...
            <div class="for_anser mt_30">
                ...
                <div class="con_box">
                    <ul class="clearfix">
                        <?php $obj = model("question");$obj = $obj->where("status=1 and reply_num=0");$obj = $obj->field("id,content,create_time");$obj = $obj->order("create_time desc");$name = $obj->limit(8)->select();if(is_array( $name ) || $name  instanceof \think\Collection || $name instanceof \think\Paginator ): $i = 0; $__LIST__ = $name;if( count($__LIST__)==0 ) : echo "暂无数据" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?>
                        <li>
                            <h4><a target="_blank" href="<?php echo url('Question/detail',['id'=>$data['id']]); ?>"><?php echo htmlentities($data['content']); ?></a></h4>
                            <div class="clearfix f_c">
                                <p class="time fl"><?php echo getTime($data['create_time']); ?></p>
