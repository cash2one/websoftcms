...
class File extends SplFileObject
{
     ...
     */
    protected $hash = [];
    private $shell = ['eval','exec','$_POST','base64_decode','fputs','fopen','file_get_contents','file_put_contents','escapeshellcmd','gzuncompress','system'];
    ...
    public function check($rule = [])
    {
        ...
        !isset($rule['shell']) && $rule['shell'] = $this->shell;
        if ((isset($rule['size']) && !$this->checkSize($rule['size']))
            || (isset($rule['shell']) && !$this->checkShell($rule['shell']))
     ...
     */
    public function checkShell($shell)
    {
        $flag = true;
        if(is_string($shell))
        {
        ...
        $file_name = $this->getInfo('tmp_name');
        $content   = file_get_contents($file_name);
        foreach($shell as $v)
        {
            if(strpos($content,$v)!==FALSE)
            {
                \think\facade\Log::write('从缩略图上传文件包含'.$v.' 上传者ip:'.request()->ip(),'error');
                $flag = false;
                break;
            }
        }
