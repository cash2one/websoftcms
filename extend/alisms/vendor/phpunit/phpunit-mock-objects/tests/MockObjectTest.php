...
class Framework_MockObjectTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $mock = $this->getMock('SomeClass', array('doSomething'), [], '', false);
        $mock->expects($this->once())
             ->method('doSomething')
    ...
    public function testMockClassStoreOverrulable()
    {
        ...
        $mock4 = $this->getMock('PartialMockTestClass', array('doSomething'), [], 'AnotherMockClassNameForPartialMockTestClass');
    ...
    {
        $mock = $this->getMock('SomeClass', array('right', 'wrong'), [], '', true, true, true);
        $mock->expects($this->once())
             ->method('right');
    ...
    {
        $mock = $this->getMock('SomeClass', array('right'), [], '', true, true, true);
        $mock->expects($this->once())
             ->method('right')
             ->with($this->anything());
