...
class OfficeRental extends UserBase
{
    ...
    {
        $where = $this->search();
        $field = "id,title,city,renovation,estate_name,img,price,acreage,status,update_time,top_time,timeout";
        ...
        $this->assign('controller','office_rental');
        return $this->fetch();
    }
    /**
     * @return mixed
    ...
    public function edit()
    {
        ...
        if(!$id){
            ...
            $join = [['office_rental_data d','d.house_id = o.id','left']];
    ...
    public function save()
    {
        ...
        $result = $this->validate($data,'app\manage\validate\OfficeRental');//调用验证器验证
        if(true !== $result)
        {
            // 验证失败 输出错误信息
            $this->error($result);
