...
class ShopsRental extends UserBase
{
    private $queryData;
    ...
    public function index()
    {
        ...
        $this->assign('controller','shops_rental');
        return $this->fetch();
    }
    /**
     * @return mixed
    ...
    public function edit()
    {
        ...
        if(!$id){
            ...
            $join = [['shops_rental_data d','d.house_id = o.id','left']];
    ...
    public function save()
    {
        ...
        $result = $this->validate($data,'app\manage\validate\ShopsRental');//调用验证器验证
        if(true !== $result)
        {
            // 验证失败 输出错误信息
            $this->error($result);
