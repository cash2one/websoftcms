...
class ShopsRental extends MobileBase
{
    private $pageSize = 10;
    ...
    public function detail()
    {
        ...
        if($id)
        {
            $where['h.id']     = $id;
            $where['h.status'] = 1;
            $obj  = model('shops_rental');
            $join = [['shops_rental_data d','h.id=d.house_id']];
            ...
            $this->setSeo($info);
            $share_title = $info['title'].$info['acreage'].config('filter.acreage_unit').$info['price'].config('filter.rental_price_unit');
            updateHits($info['id'],'shops_rental');
