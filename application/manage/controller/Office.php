...
class Office extends ManageBase
{
    ...
    public function delete()
    {
        \app\common\model\Office::event('after_delete',function($obj){
           ...
           //删除推荐数据
            ...
            \org\Relation::deleteByHouse($obj->id,'office');
        });
        parent::delete();
    }
            ...
            try{
                ...
                if(isset($data['id']))
                {
                    if($obj->allowField(true)->save($data,['id'=>$data['id']]))
                    {
                        ...
                        $return['msg'] = '编辑房源信息成功';
                        $return['code'] = 1;
                        \org\Relation::addMetro('office',$data['lng'],$data['lat'],$data['id'],$data['city']);
                    }else{
                        $return['msg'] = '编辑房源信息失败！';
                    }
                }else{
                    if($obj->allowField(true)->save($data))
                    {
                        ...
                        $return['msg'] = '添加房源信息成功';
                        $return['code'] = 1;
                        \org\Relation::addMetro('office',$data['lng'],$data['lat'],$house_id,$data['city']);
                    }else{
                        $return['msg'] = '添加房源信息失败！';
                    }
                }
    ...
    private function optionHouseData($house_id,$data,$update = false)
    {
        ...
        {
            $info['seo_keys'] = $data['estate_name'].'写字楼出售';
        }else{
