...
//前置操作定义
    ...
    public function addDo(){
        ...
        $py = new \org\Pinyin();
        //检测分类是否存在
        if($this->mod->name_exists($data['name'], $data['pid'],0,$data['province_id'])){
            $this->error('操作失败，城市 '.$data['name'].' 已存在');
        }else{
            //生成spid
            $data['lat']  = $lat;
            $data['lng']  = $lng;
            ...
            ($data['alias'] && isset($data['domain']) && !$data['domain']) && $data['domain'] = $data['alias'];
            if(isset($data['domain']) && $data['domain'] && $this->mod->where('domain',$data['domain'])->count() > 0)
            {
                $this->error('该域名已存在');
            }
        }
    ...
    public function doCache(){
        $lists = $this->mod->field('id,pid,name,alias,is_hot,domain,lat,lng')->where('status',1)->order(['ordid'=>'asc','id'=>'desc'])->select();
        if($lists){
