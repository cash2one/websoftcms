...
class OfficeRental extends ManageBase
{
    private $model = 'office_rental';
    ...
    {
        \app\common\model\Office::event('after_delete',function($obj){
            //删除扩展数据
           ...
           //删除推荐数据
            ...
            \org\Relation::deleteByHouse($obj->id,'office_rental');
        });
        parent::delete();
    }
    ...
    {
        $data = input('post.');
        $result = $this->validate($data,'Office');//调用验证器验证
        $return['code']   = 0;
        $return['msg']    = '';
        ...
        if(true !== $result)
        {
            ...
            try{
                ...
                {
                    //price_type = 1，填写月租总价 price_type = 2 填写月租单价
                    if($data['price_type'] == 1)
                    {
                        //计算均价
                        $data['average_price'] = ceil($data['price'] / $data['acreage']);
                    }else{
                        //计算总价
                        $data['average_price'] = $data['price'];
                        $data['price'] = $data['average_price'] * $data['acreage'];
                    }
                }
                ...
                if(isset($data['id']))
                {
                    if($obj->allowField(true)->save($data,['id'=>$data['id']]))
                    {
                        ...
                        $return['msg'] = '编辑房源信息成功';
                        $return['code'] = 1;
                        \org\Relation::addMetro('office_rental',$data['lng'],$data['lat'],$data['id'],$data['city']);
                    }else{
                        $return['msg'] = '编辑房源信息失败！';
                    }
                }else{
                    if($obj->allowField(true)->save($data))
                    {
                        ...
                        $return['msg'] = '添加房源信息成功';
                        $return['code'] = 1;
                        \org\Relation::addMetro('office_rental',$data['lng'],$data['lat'],$house_id,$data['city']);
                    }else{
                        $return['msg'] = '添加房源信息失败！';
                    }
                }
    ...
    private function optionHouseData($house_id,$data,$update = false)
    {
        ...
        {
            $info['seo_keys'] = $data['estate_name'].'写字楼出租';
        }else{
