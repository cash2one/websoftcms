...
class ShopsRental extends ManageBase
{
    private $model = 'shops_rental';
    ...
    {
        $data = input('post.');
        $result = $this->validate($data,'ShopsRental');//调用验证器验证
        $return['code']   = 0;
        $return['msg']    = '';
    ...
    {
        \app\common\model\ShopsRental::event('after_delete',function($obj){
            if($obj->img && strpos($obj->img,'://')===false)
            {
                //删除数据同时删除图片
                $img = '.'.$obj->img;
            ...
            {
                model('attachment')->deleteAttachment($info['info'],$info['file']);
            }
           //删除推荐数据
    ...
    private function optionHouseData($house_id,$data,$update = false)
    {
        ...
        {
            $info['seo_keys'] = $data['estate_name'].'商铺出租,商铺转让';
        }else{
