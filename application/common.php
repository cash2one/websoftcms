...
function getCity($field = 'tree')
{
    ...
    if(!$city)
    {
        $lists = model('city')->field('id,pid,name,alias,is_hot,img,domain,lat,lng')->where('status',1)->order(['ordid'=>'asc','id'=>'desc'])->select();
        if($lists){
 ...
 * 二手房价格
 */
function  getSecondPrice($key = '',$field = 'price'){
    $data = config('filter.second_price');
    try{
        if($key){
            $len = count($data);
            if($key == $len){
