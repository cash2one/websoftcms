...
class House extends AgentBase
{
    ...
    {
        $this->list_field = 'id,city,title,img,video,price,unit,red_packet,create_time,update_time,status,ordid,rec_position';
        ...
        if(true !== $result)
        {
            ...
            try{
                ...
                if($obj->allowField(true)->save($data))
                {
                    ...
                    model('house_price')->addHousePrice($house_id,$data['price']);
    ...
    public function editDo()
    {
        ...
        if(true !== $result)
        {
            ...
            try{
                ...
                //添加价格并计算涨幅比
                $rate = model('house_price')->addHousePrice($data['id'],$data['price']);
    ...
    public function updateHouseData($house_id,$data)
    {
        ...
        if(!model('house_data')->where('house_id',$house_id)->find())
        {
            //不存在扩展数据则添加
