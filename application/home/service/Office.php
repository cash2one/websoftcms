...
class Office
{
    public function lists($zoom,$city = 0)
    {
        ...
        $field  = 'id,title,city,estate_name,type,img,renovation,price,average_price,acreage,tags,lng,lat,estate_id';
        $obj    = model('office');
        $mod    = $obj->where($where)
            ->field($field)
        ...
        if($lists)
        {
            foreach($lists as &$v)
            {
                ...
                $v['w_url']      = "/pages/office/detail/index?id=".$v['id'];
            ...
            if($estate)
            {
                ...
                foreach($estate as &$val)
                {
                    ...
                    $val['url'] = url('Office/index',['estate_id'=>$val['id']]);
                    $val['w_url'] = "/pages/office/index/index?estate_id=".$val['id'];
                }
            }
            $return['code']   = 1;
            $return['data']   = $lists;
