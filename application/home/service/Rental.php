...
class Rental
{
    public function lists($zoom,$city = 0)
    {
        ...
        $field  = 'id,title,city,estate_name,room,living_room,img,renovation,price,acreage,tags,lng,lat,estate_id';
        $obj    = model('rental');
        $mod    = $obj->where($where)
                 ->field($field)
        ...
        if($lists)
        {
            foreach($lists as &$v)
            {
                ...
                $v['w_url'] = "/pages/rental/detail/index?id=".$v['id'];
            ...
            if($estate)
            {
                ...
                foreach($estate as &$val)
                {
                    ...
                    $val['url'] = url('Rental/index',['estate_id'=>$val['id']]);
                    $val['w_url'] = "/pages/rental/index/index?estate_id=".$val['id'];
                }
            }
            $return['code']   = 1;
            $return['data']   = $lists;
    ...
    private function search($city)
    {
        ...
        $param['renovation'] = input('param.renovation',0);//装修
        $param['rental_type'] = input('param.rental_type/d',0);
