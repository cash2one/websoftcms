...
class Office extends HomeBase
{
    ...
    public function index()
    {
        ...
        $this->assign('hot',$this->getHotOffice());//热门房源
    ...
    private function getLists()
    {
        ...
        if(isset($where['m.metro_id']) || isset($where['m.station_id']))
        {
            ...
            $lists = $obj->join($join)->where($where)->where('m.model','office')->where('o.top_time','lt',$time)->field($field)->group('o.id')->order($this->getSort($sort))->paginate(10,false,['query'=>['keyword'=>$keyword]]);
        }else{
        ...
        if($lists->currentPage() == 1)
        {
            ...
            if(isset($where['m.metro_id']) || isset($where['m.station_id']))
            {
                $field .= ',m.metro_name,m.station_name,m.distance';
                $join  = [['metro_relation m','m.house_id = o.id']];
                $obj->join($join)->where('m.model','office')->group('o.id');
            }
    ...
    private function search()
    {
        ...
        if($param['search_type'] == 2)
        {
            ...
            if(!empty($param['area']))
            {
                $data[] = ['o.city','in',$this->getCityChild($param['area'])];
        ...
        {
            $data[] = getBussinessCondition('office_price','o.average_price',$param['price']);
            $price  = config('filter.office_price');
        ...
        if($citySeo)
        {
            $seo_title = $citySeo['seo_title'].'写字楼出售'.$seo_title;
            $seo_keys  = $citySeo['seo_keys'].'写字楼出售,'.$seo_keys;
    ...
    private function getHotOffice($num = 5)
    {
        ...
        $city = $this->getCityChild();
        $city && $where[] = ['city','in',$city];
        $lists = model('office')->field('id,title,img,price,estate_name,acreage')->where($where)->order('hits desc,id desc')->limit($num)->select();
        return $lists;
    }
    /**
     * @param $sort
