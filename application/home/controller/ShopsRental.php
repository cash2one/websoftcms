...
class ShopsRental extends HomeBase
{
    private $mod = 'shops_rental';
    ...
    public function index()
    {
        ...
        $this->assign('lease_type',getLeaseType());//商铺类别
    ...
    private function getLists()
    {
        ...
        $field = "id,title,estate_name,contact_name,user_type,update_time,img,price,average_price,tags,renovation,address,acreage,type,floor,total_floor,lease_type,transfer_fee";
        $obj   = model($this->mod);
        ...
        {
            $top = $obj->where($where)->where('top_time','gt',$time)->field($field)->order(['top_time'=>'desc','id'=>'desc'])->select();
        }else{
            $top = false;
        }
    ...
    private function search()
    {
        ...
        $param['lease_type']    = input('param.lease_type/d',0);//商铺类别
        ...
        if($param['lease_type'])
        {
            $data['lease_type'] = $param['lease_type'];
        }
        if($keyword)
        {
        ...
        {
            $data[] = getBussinessCondition('shops_rental_price','price',$param['price']);
            $price  = config('filter.shops_rental_price');
        ...
        if($citySeo)
        {
            $seo_title = $citySeo['seo_title'].'商铺出租'.$seo_title;
            $seo_keys  = $citySeo['seo_keys'].'商铺出租,'.$seo_keys;
