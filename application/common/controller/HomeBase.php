...
class HomeBase extends \think\Controller
{
    ...
    private function getDomainByCity($second){
        if($second)
        {
            $info =  model('city')->field('id,name,lng,lat,domain,seo_title,seo_keys,seo_desc')->where(['domain'=>$second,'status'=>1,'pid'=>0])->find();
            if(!$info){
                $this->redirect('Error/index',['url'=>base64_encode('//'.config('url_domain_root'))]);
            }
        }else{
    ...
    {
        $info =  model('city')->field('id,name,domain,seo_title,seo_keys,seo_desc')->where(['status'=>1,'pid'=>0])->order(['ordid'=>'asc','id'=>'asc'])->find();
        return $info;
    }
    /**
     * 根据城市Id获取相关城市信息
     */
    protected function getCityInfoById($id)
    {
        if(cache('city_'.$id))
        {
            $city = cache('city_'.$id);
        }else{
            $city = model('city')->where('id',$id)->field('id,name,seo_title,seo_keys,seo_desc')->find();
            if(empty($city['seo_title']))
            {
                $city['seo_title'] = $city['name'];
            }
            $city = $city->toArray();
        }
        if($city){
