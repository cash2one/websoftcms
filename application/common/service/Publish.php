...
class Publish
{
    public static function save($model,$data,$user_id,$user_model=1)
    {
        $return['code']   = 0;
        $return['msg']    = '';
        $obj = model($model);
        ...
        try{
            $setting = getUserCate();
            $setting = $setting[$user_model]['data'];//用户设置
            $status  = isset($setting['check_'.$model])?$setting['check_'.$model]:1;
