...
class OfficeRental extends ApiBase
{
    private $pageSize = 15;
    ...
    public function read($id)
    {
        ...
        if($id)
        {
            ...
            $where['h.status'] = 1;
            $obj  = model('office_rental');
            $join = [['office_rental_data d','h.id=d.house_id']];
            $field = "h.*,d.info,d.file";
            ...
            if($info)
            {
                ...
                $info['tags']         = $this->getTags(16,explode(',',$info['tags'],30));
                ...
                $info['grade']        = getLinkMenuName(17,$info['grade']);
                $info['floor']        = getLinkMenuName(7,$info['floor']);
                $info['type']         = getLinkMenuName(15,$info['type']);
                $info['update_time']  = getTime($info['update_time'],'mohu').'更新';
                updateHits($info['id'],'office_rental');
                $return['code'] = 200;
            }
            $return['data'] = $info;
        }else{
     ...
     * 筛选条件
     */
    public function officeAttr()
    {
        $city               = getCity();
        $data['city']       = isset($city[$this->city]['_child']) ? $city[$this->city]['_child'] : $city;
        $data['price']      = getBussinessCondition('office_price');
        $data['type']       = getLinkMenuCache(15);
        $data['tags']       = getLinkMenuCache(16);
        $data['renovation'] = getLinkMenuCache(8);
        $data['acreage']    = getBussinessCondition("office_acreage");
        $data['sort']       = [
            0 => ['id'=>0,'name'=>'默认'],
            1 => ['id'=>1,'name'=>'总价从低到高'],
            2 => ['id'=>2,'name'=>'总价从高到低'],
    ...
    private function search()
    {
        ...
        {
            $data[] = getBussinessCondition('office_price','average_price',$param['price']);
        }
