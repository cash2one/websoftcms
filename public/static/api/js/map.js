...
var BMapApplication = {
    ...
    'setBmapContainer' : function (elemId){
        ...
        this.map = (this.map == null) ? new BMap.Map(elemId,{ enableMapClick: false,minZoom : 10 }) : this.map;
    },
    /* 清空基本地图容器 */
                ...
                if(result.code == 1 && result.data)
                {
                    if(result.zoom < 13)
                    {
                        ...
                        $(data).each(function (i, ite) {
                            ...
                            var txt = ite.title, mouseoverTxt = txt + " " + ite.price+unit;
                            var point = new BMap.Point(ite.lng,ite.lat);
                            pointArr.push(point);
                            that.setComplexPrototype();
                            var myCompOverlay = new that.ComplexCustomOverlay(point,mouseoverTxt,mouseoverTxt,ite.sale_status,ite.w_url);
                            that.map.addOverlay(myCompOverlay);
                        });
                    }
                    //按楼盘名称搜索所有结果显示在可视区域内
           ...
           // if(_this.targetUrl)
            //{
                BMapLib.EventWrapper.addDomListener(div, "touchend", function(e){
                    wx.miniProgram.navigateTo({url: that._url});
                });
           // }
